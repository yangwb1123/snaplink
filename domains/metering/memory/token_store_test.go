...
func recordEvent(t *testing.T, s *Store, clientID string, kind metering.Kind, endpoint metering.Endpoint, when time.Time) {
	t.Helper()
	if err := s.Record(context.Background(), metering.Event{
		ClientID: clientID,
		Kind:     kind,
		Endpoint: endpoint,
		At:       when,
...
func TestStore_BucketsByMinuteClientKindEndpoint(t *testing.T) {
	...
	recordEvent(t, s, "client-a", metering.KindAccess, metering.EndpointToken, at(10, 0, 0))
	recordEvent(t, s, "client-a", metering.KindAccess, metering.EndpointToken, at(10, 0, 59))
	// Next minute — a second bucket.
	recordEvent(t, s, "client-a", metering.KindAccess, metering.EndpointToken, at(10, 1, 0))
	// Different client — a third bucket, same minute as the first pair.
	recordEvent(t, s, "client-b", metering.KindAccess, metering.EndpointToken, at(10, 0, 30))
	// Different kind — a fourth bucket.
	recordEvent(t, s, "client-a", metering.KindRefresh, metering.EndpointToken, at(10, 0, 15))
	// Different endpoint — a fifth bucket.
	recordEvent(t, s, "client-a", metering.KindAccess, metering.EndpointIntrospect, at(10, 0, 45))

	buckets, err := s.Query(ctx, metering.Query{})
	if err != nil {
	...
	for _, b := range buckets {
		if b.ClientID == "client-a" && b.Kind == metering.KindAccess &&
			b.Endpoint == metering.EndpointToken && b.Minute.Equal(metering.BucketMinute(at(10, 0, 0))) {
...
func TestStore_EvictsOldestOnCap(t *testing.T) {
	...
	recordEvent(t, s, "c", metering.KindAccess, metering.EndpointToken, at(9, 0, 0))
	recordEvent(t, s, "c", metering.KindAccess, metering.EndpointToken, at(9, 1, 0))
	recordEvent(t, s, "c", metering.KindAccess, metering.EndpointToken, at(9, 2, 0))
	...
	recordEvent(t, s, "c", metering.KindAccess, metering.EndpointToken, at(9, 3, 0))
	if got := s.TrackedBuckets(); got != 3 {
	...
	for _, b := range buckets {
		if b.Minute.Equal(metering.BucketMinute(at(9, 0, 0))) {
	...
	recordEvent(t, s, "c", metering.KindAccess, metering.EndpointToken, at(9, 3, 30))
	if got := s.TrackedBuckets(); got != 3 {
...
func TestStore_QueryFiltersByClientAndHalfOpenWindow(t *testing.T) {
	s := New()
	ctx := context.Background()
	recordEvent(t, s, "a", metering.KindAccess, metering.EndpointToken, at(8, 0, 0))
	recordEvent(t, s, "b", metering.KindAccess, metering.EndpointToken, at(8, 0, 0))
	recordEvent(t, s, "a", metering.KindAccess, metering.EndpointToken, at(8, 1, 0))
	recordEvent(t, s, "a", metering.KindAccess, metering.EndpointToken, at(8, 2, 0))

	buckets, err := s.Query(ctx, metering.Query{ClientID: "a"})
	if err != nil {
		t.Fatalf("Query: %v", err)
	}
	if len(buckets) != 3 {
	...
	windowed, err := s.Query(ctx, metering.Query{
		ClientID: "a",
		Since:    at(8, 0, 0),
		Until:    at(8, 2, 0),
	})
	...
	for _, b := range windowed {
		if b.Minute.Equal(metering.BucketMinute(at(8, 2, 0))) {
