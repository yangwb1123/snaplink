...
// This file exercises metering.HandleAdminUsage against a REAL memory
// Store (no mocks, per AGENTS.md §0.5) — it lives beside the memory
// implementation rather than in domains/metering itself because
// metering/memory already depends on metering one-way; the reverse
// import from a domains/metering test would be a cycle.
...
func seedStore(t *testing.T, s *Store, clientID string, kind metering.Kind, endpoint metering.Endpoint, minuteOffset int) {
	t.Helper()
...
func TestHandleAdminUsage_ReturnsAggregatedBuckets(t *testing.T) {
	store := New()
	seedStore(t, store, "client-a", metering.KindAccess, metering.EndpointToken, 0)
	seedStore(t, store, "client-b", metering.KindRefresh, metering.EndpointIntrospect, 1)

	ctx, rec := newAdminCtx(t, "/admin/tokens/usage")
	metering.HandleAdminUsage(store, spi.NopLogger{}, ctx)
...
func TestHandleAdminUsage_FiltersByClientID(t *testing.T) {
	store := New()
	seedStore(t, store, "client-a", metering.KindAccess, metering.EndpointToken, 0)
	seedStore(t, store, "client-b", metering.KindAccess, metering.EndpointToken, 0)
...
func TestHandleAdminUsage_SinceUntilWindowFilters(t *testing.T) {
	store := New()
	seedStore(t, store, "client-a", metering.KindAccess, metering.EndpointToken, 0)
	seedStore(t, store, "client-a", metering.KindAccess, metering.EndpointToken, 5)
