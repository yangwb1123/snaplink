...
//
// Separation of Duty (see sod.go) is modeled as two optional Provider
// extensions, additive over the base contract: SoDProvider declares Static
// SoD (SSoD) role sets that may never be held together (enforced inline by
// AssignRoles/AddRoleToUser), and SessionRoleActivator adds Dynamic SoD
// (DSoD) — a session-scoped ACTIVE subset of a subject's held roles,
// distinct from the full ASSIGNED set, so a subject may hold two roles
// that are mutually exclusive to activate in the same session.
package permissions
