// Package memory is the in-memory [tokenusage.Store]: per-minute
...
// [tokenusage.Store]. Safe for concurrent use.
type Store struct {
	mu      sync.Mutex
	max     int
...
func (s *Store) Record(_ context.Context, ev tokenusage.Event) error {
	key := bucketKey{
		minute:   tokenusage.BucketMinute(ev.At).Unix(),
...
func (s *Store) Query(_ context.Context, q tokenusage.Query) ([]tokenusage.Bucket, error) {
	s.mu.Lock()
	out := make([]tokenusage.Bucket, 0)
	for key, count := range s.buckets {
		if !matches(key, q) {
			continue
		}
		out = append(out, tokenusage.Bucket{
...
func matches(key bucketKey, q tokenusage.Query) bool {
	if q.ClientID != "" && key.clientID != q.ClientID {
		return false
	}
	if !q.Since.IsZero() && key.minute < tokenusage.BucketMinute(q.Since).Unix() {
		return false
	}
	// Until is exclusive: a bucket AT the until minute is out.
	if !q.Until.IsZero() && key.minute >= tokenusage.BucketMinute(q.Until).Unix() {
		return false
	}
	return true
}

var _ tokenusage.Store = (*Store)(nil)
var _ tokenusage.TrackedBucketReporter = (*Store)(nil)
