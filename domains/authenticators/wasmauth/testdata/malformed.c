// malformed.c -- test fixture whose authenticate export satisfies the
// ABI's SIGNATURE but returns bytes that are NOT valid JSON, so
// authenticate's json.Unmarshal of the Result must fail (fail-closed:
// see ../authenticate.go). See policy.c for the toolchain rationale.
//
