...
ORDER BY linked_at, id`, userID, identitylink.StatusActive)
	if err != nil {
		return nil, fmt.Errorf("identitylink/sqlite: list: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("identitylink/sqlite: list scan: %w", err)
		}
		out = append(out, rec)
	}
	return out, rows.Err()
...
VALUES (?, ?, ?, ?, ?, ?, 0)`,
		...
		if err != nil {
			return identitylink.Identity{}, fmt.Errorf("identitylink/sqlite: link: %w", err)
		}
	...
	return identitylink.Identity{}, errors.New("identitylink/sqlite: concurrent link changed repeatedly")
}
...
WHERE provider = ? AND subject = ? AND status = ?`,
	...
	if err != nil {
		return identitylink.Identity{}, false, fmt.Errorf("identitylink/sqlite: find: %w", err)
	}
	return rec, true, nil
}
