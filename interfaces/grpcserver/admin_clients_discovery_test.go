...
func TestClientAdmin_DiscoveryChangeTrigger(t *testing.T) {
	...
	svc := grpcserver.NewClientAdminService(store, nil, func() { calls.Add(1) }, nil)
...
// TestClientAdmin_ClientChangeTrigger pins which mutations fire the
// per-login client-cache invalidation callback: Create/Update/Delete AND
...
func TestClientAdmin_ClientChangeTrigger(t *testing.T) {
	...
	svc := grpcserver.NewClientAdminService(store, nil, nil, func(id string) {
		evictions.Add(1)
		lastID.Store(id)
	})
	ctx := context.Background()
	...
	if _, err := svc.Create(ctx, &adminv1.CreateClientRequest{
		Client: &adminv1.Client{Id: "c2", Active: true},
	}); err != nil {
		t.Fatalf("create: %v", err)
	}
	if _, err := svc.Update(ctx, &adminv1.UpdateClientRequest{
	...
	// Create + Update + RotateSecret + Delete = 4 evictions.
	if got := evictions.Load(); got != 4 {
		t.Fatalf("client-change callback fired %d times, want 4 (Create/Update/RotateSecret/Delete)", got)
	}
	if id, _ := lastID.Load().(string); id != "c2" {
		t.Fatalf("last evicted id = %q, want c2 (the Delete)", id)
	}
}
