...
// TestGinRouter_UnmatchedByteIdenticalToHTTPNotFound pins the constructor's
...
// all answer http.NotFound's exact bytes — in particular /known/ must be a
// 404, never gin's native 301 trailing-slash redirect. Deleting the
// normalization code in the constructor makes this test (and the shared
// conformance suite) fail immediately.
func TestGinRouter_UnmatchedByteIdenticalToHTTPNotFound(t *testing.T) {
	t.Parallel()
	r := NewGinRouter()
	...
	for _, tc := range []struct {
		...
		{"trailing slash is 404, not 301", http.MethodGet, "/known/"},
	} {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			rec := do(t, r, tc.method, tc.path, "")
			if rec.Code != ref.Code || rec.Body.String() != ref.Body.String() || !headersEqualGin(rec.Header(), ref.Header()) {
...
func headersEqualGin(a, b http.Header) bool {
	if len(a) != len(b) {
		return false
	}
	for k, va := range a {
...
// TestGinRouter_UseAfterRegisterDoesNotAffectRegisteredRoutes pins the
...
// registered after must. The pre-fix adapter read g.middlewares at request
// time and retroactively mutated registered routes — this test was red
// then.
func TestGinRouter_UseAfterRegisterDoesNotAffectRegisteredRoutes(t *testing.T) {
	t.Parallel()
	r := NewGinRouter()
...
// TestGinRouter_ConcurrentUseAndServeHTTP is the -race tripwire for the
// snapshot design: the request path never reads g.middlewares (each route
...
func TestGinRouter_ConcurrentUseAndServeHTTP(t *testing.T) {
	t.Parallel()
	r := NewGinRouter()
