...
func geoFromContext(ctx HandlerContext) *core.GeoInfo {
	if g, ok := GeoFromHandlerContext(ctx); ok && g != nil { return g }; return nil
}
func uaSummary(ua string) string {
	if ua == "" { return "" }
	dp := device.ParseUserAgent(ua)
	if n := dp.DeviceName; n != "" { if p := dp.Platform; p != "" { return p + " · " + n }; return n }
	if b := dp.BrowserName; b != "" { if p := dp.Platform; p != "" { return p + " · " + b }; return b }
	if p := dp.Platform; p != "" { return p }
	return string(dp.Type)
}
func locationSummary(ctx HandlerContext) string {
	g := geoFromContext(ctx); if g == nil { return "" }
	if g.City != "" && g.Region != "" { return g.City + ", " + g.Region }
	if g.City != "" { return g.City }; if g.CountryCode != "" { return g.CountryCode }
	return ""
}
func deviceCtxFrom(ctx HandlerContext) *deviceContext {
	v := ctx.Get("device_ctx"); dc, _ := v.(*deviceContext); return dc
}
func deviceTypeFromCtx(ctx HandlerContext) string {
	dc := deviceCtxFrom(ctx); if dc == nil { return "" }; return dc.Type
}
func computeDeviceTrustScore(secCtx *device.LoginSecurityContext, loginCount int, existing *device.Device) float64 {
	var score float64
	switch {
	case secCtx != nil && secCtx.DeviceIsNew: score = 0.3
	case secCtx != nil && secCtx.LocationIsNew: score = 0.4
	case existing != nil: score = existing.TrustScore
	default: score = 0.5
	}
	if loginCount > 1 { score += float64(loginCount-1) * 0.02 }
	if existing != nil && loginCount >= 10 { score = 0.85 }
...
func deviceAwareTTL(clientTTL time.Duration, dc *deviceContext, minTTL time.Duration) time.Duration {
	if dc == nil || dc.SecurityCtx == nil || clientTTL <= 0 { return clientTTL }
	if !dc.SecurityCtx.DeviceIsNew && !dc.SecurityCtx.LocationIsNew { return clientTTL }
	r := time.Hour
	if minTTL > 0 && r < minTTL { r = minTTL }
	if r < clientTTL { return r }
	return clientTTL
}
func appendTrustHistory(existing *device.Device, e device.TrustHistoryEntry) []device.TrustHistoryEntry {
	if existing == nil || len(existing.TrustHistory) == 0 { return []device.TrustHistoryEntry{e} }
...
func (s *Server) registerLoginDevice(ctx HandlerContext, userID string) *deviceContext {
	if s.deviceStore == nil { return nil }
	...
	if dec := device.EvaluatePolicy(s.devicePolicy, s.deviceStore, userID, fp); !dec.Allowed {
		s.logger.Info("device policy rejected login", "user", userID, "reason", dec.Reason); return nil
	}
	ip := audit.ClientIP(ctx.Request())
	secCtx := device.BuildSecurityContext(s.deviceStore, func(uid string) (int, error) {
		if s.sessionMgr == nil { return 0, nil }
		sessions, err := s.sessionMgr.ListByUser(ctx.Request().Context(), uid)
		if err != nil { return 0, err }; return len(sessions), nil
	}, userID, fp, ip)
	if secCtx != nil && secCtx.PreviousLogin != nil { secCtx.PreviousLogin.Location = locationSummary(ctx) }
	...
	if existing, err := s.deviceStore.GetByFingerprint(ctx.Request().Context(), userID, fp); err == nil && existing != nil {
		...
		if d := int(time.Since(existing.LastSeenAt).Hours()/24); d > 0 { existingDevice.TrustScore = device.DecayTrustScore(existing.TrustScore, d) }
	}
	estimatedCount := 1; if existingDevice != nil { estimatedCount = existingDevice.LoginCount + 1 }
	...
	if err := s.deviceStore.Upsert(ctx.Request().Context(), d); err != nil {
		s.logger.Error("device: failed to register", "user", userID, "error", err); return nil
	}
...
func (s *Server) recordLoginHistory(ctx HandlerContext, result *AuthResult, devID string, dc *deviceContext) {
	...
	ts := float64(0); if dc != nil { ts = dc.TrustScore }
...
func (s *Server) recordLoginSecurityEvents(ctx HandlerContext, result *AuthResult, clientID string, dc *deviceContext) {
	if s.auditor == nil || dc == nil || dc.SecurityCtx == nil { return }
	sec := dc.SecurityCtx
	if sec.DeviceIsNew {
		e := audit.EventFromRequest(ctx); e.Type = audit.EventNewDeviceLogin; e.Outcome = audit.OutcomeSuccess
		e.ActorID = result.UserID; e.ClientID = clientID; s.auditor.Record(ctx.Request().Context(), e)
	}
	if sec.LocationIsNew && sec.PreviousLogin != nil && sec.PreviousLogin.IP != "" {
		e := audit.EventFromRequest(ctx); e.Type = audit.EventNewLocation; e.Outcome = audit.OutcomeSuccess
		e.ActorID = result.UserID; e.ClientID = clientID; s.auditor.Record(ctx.Request().Context(), e)
	}
}
func (s *Server) applyDeviceTrustDecay(ctx HandlerContext, existing, existingDevice *device.Device, userID string, days int) {
	...
	if oldScore-existingDevice.TrustScore > 0.2 && s.auditor != nil {
		e := audit.EventFromRequest(ctx); e.Type = audit.EventTrustDecay; e.Outcome = audit.OutcomeSuccess
		e.ActorID = userID; s.auditor.Record(ctx.Request().Context(), e)
	}
}
func (s *Server) loginPageURIForClient(ctx HandlerContext, clientID string) string {
	if clientID == "" || s.clientStore == nil { return "" }
	c, err := s.clientStore.Get(ctx.Request().Context(), clientID)
	if err != nil || c == nil { return "" }; return c.LoginPageURI
}
func (s *Server) brandingForClient(ctx HandlerContext, clientID string) map[string]string {
	if clientID == "" || s.clientStore == nil || s.tenantStore == nil { return nil }
	c, err := s.clientStore.Get(ctx.Request().Context(), clientID)
	if err != nil || c == nil || c.TenantID == "" { return nil }
	t, err := s.tenantStore.GetTenant(ctx.Request().Context(), c.TenantID)
	if err != nil || t == nil { return nil }
	return t.Settings
}
func (s *Server) providersForClient(ctx HandlerContext, clientID string) []providerInfo {
	var builtins []string
	for name := range s.authenticators { builtins = append(builtins, name) }
	var tenantID string
	if clientID != "" && s.clientStore != nil {
		if c, err := s.clientStore.Get(ctx.Request().Context(), clientID); err == nil {
			...
			if len(c.AllowedAuthenticators) > 0 {
				var f []string
				for _, n := range builtins { if c.IsAuthenticatorAllowed(n) { f = append(f, n) } }
				builtins = f
			}
			if len(c.AllowedProviderIDs) > 0 { return s.filteredProviders(ctx, builtins, c.AllowedProviderIDs, tenantID) }
		}
	}
...
func (s *Server) filteredProviders(ctx HandlerContext, builtins []string, allowIDs []string, tnID string) []providerInfo {
	...
	for _, id := range allowIDs { allowSet[id] = true }
	out := make([]providerInfo, 0, len(allowIDs))
	for _, n := range builtins {
		if allowSet[n] { out = append(out, providerInfo{ID: n, Type: string(provider.TypeBuiltin), DisplayName: displayNameForBuiltin(n), Builtin: true}) }
	}
	for _, p := range s.providerStoreProviders(ctx, tnID) { if allowSet[p.ID] { out = append(out, p) } }
	return out
}
func (s *Server) providerStoreProviders(ctx HandlerContext, tnID string) []providerInfo {
	if s.providerStore == nil { return nil }
	list, err := s.providerStore.ListByTenant(ctx.Request().Context(), tnID)
	if err != nil || len(list) == 0 { return nil }
...
func displayNameForBuiltin(name string) string {
	switch name {
	case "password": return "Password"; case "webauthn": return "Passkey"; case "totp": return "Authenticator App"
	default: return name
	}
}
func (s *Server) respondLoginProviders(ctx HandlerContext, req *login.Request) bool {
	...
	if lp := s.loginPageURIForClient(ctx, req.ClientID); lp != "" { resp[KeyLoginPageURI] = lp }
	if b := s.brandingForClient(ctx, req.ClientID); b != nil { resp[KeyBranding] = b }
	ctx.JSON(http.StatusOK, resp); return true
}
func trackSessionActivity(ctx context.Context, mgr core.SessionManager, sid string) {
	if mgr == nil || sid == "" { return }
	tracker, ok := mgr.(core.SessionActivityTracker)
	if !ok { return }; tracker.TrackActivity(ctx, sid)
}
