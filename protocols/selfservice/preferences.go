...
// carries credential material (password_hash*, seeded_password, scim:*,
...
func HandleMyPreferencesGet(d Deps, ctx core.HandlerContext, userID string) {
	...
	for _, key := range preferenceOrder {
		...
		if v := user.Attributes[key]; v != "" {
			if preferenceValidators[key](v) {
				out[key] = v
			}
		}
	}
...
// write a stray attribute). Values are validated per-key (see
// preferenceValidators). A key explicitly set to "" is removed.
func HandleMyPreferencesPut(d Deps, ctx core.HandlerContext, userID string) {
	middleware.TokenNoStoreHeaders(ctx)
	var req map[string]string
	if err := oauth.BindParams(ctx, &req); err != nil || req == nil {
	...
	for key, value := range req {
		...
		if !allowlisted {
			// Unknown key: fail closed rather than writing a stray attribute.
			ctx.JSON(http.StatusBadRequest, d.ErrorBody(core.ErrInvalidRequest))
			return
		}
