...
func TestSessionTokenIssuer_RoundTrip(t *testing.T) {
	...
	tok, err := s.Issue(context.Background(),
		&sso.Subject{ID: "u-alice", Claims: map[string]string{"role": "admin"}},
		[]string{"read", "write"},
	)
	if err != nil {
...
func TestSessionTokenIssuer_TokensAreUnique(t *testing.T) {
	...
	for range 50 {
		tok, err := s.Issue(context.Background(), &sso.Subject{ID: "u"}, nil)
		if err != nil {
			t.Fatalf("Issue: %v", err)
		}
...
func TestSessionTokenIssuer_IssueRequiresSubject(t *testing.T) {
	...
	if _, err := s.Issue(context.Background(), &sso.Subject{}, nil); err == nil {
...
func TestSessionTokenIssuer_ExpiredTokenDeleted(t *testing.T) {
	...
	tok, _ := s.Issue(context.Background(), &sso.Subject{ID: "u"}, nil)
