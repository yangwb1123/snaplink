...
// request objects. Delegates that also publish a key ([core.JWKSProvider])
...
//
// Implements [security.JWEDecrypter] and [core.JWKSProvider].
type MultiJWEDecrypter struct {
	byAlg         map[string]security.JWEDecrypter
	algs          []string
	encs          []string
	jwksProviders []core.JWKSProvider
}
...
func NewMultiJWEDecrypter(decrypters ...security.JWEDecrypter) *MultiJWEDecrypter {
	...
	for _, d := range decrypters {
		...
		if p, ok := d.(core.JWKSProvider); ok {
...
var (
	...
	_ core.JWKSProvider     = (*MultiJWEDecrypter)(nil)
)
...
func (m *MultiJWEDecrypter) JWKS(ctx context.Context) ([]core.JWK, error) {
	var out []core.JWK
