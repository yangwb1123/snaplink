...
type CompositeMFAEnrollmentStore struct {
	delegates []core.MFAEnrollmentStore
}

// NewCompositeMFAEnrollmentStore composes delegates into one MFAEnrollmentStore.
// If a delegate also implements core.TOTPEnrollmentWriter, the returned store
...
func NewCompositeMFAEnrollmentStore(delegates ...core.MFAEnrollmentStore) core.MFAEnrollmentStore {
	base := &CompositeMFAEnrollmentStore{delegates: delegates}
	for _, d := range delegates {
		if w, ok := d.(core.TOTPEnrollmentWriter); ok {
...
type compositeMFAWithTOTP struct {
	*CompositeMFAEnrollmentStore
	writer core.TOTPEnrollmentWriter
}
...
var (
	_ core.MFAEnrollmentStore   = (*CompositeMFAEnrollmentStore)(nil)
	_ core.MFAEnrollmentStore   = (*compositeMFAWithTOTP)(nil)
	_ core.TOTPEnrollmentWriter = (*compositeMFAWithTOTP)(nil)
)
