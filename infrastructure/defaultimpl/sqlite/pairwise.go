...
func (s *PairwiseSubjectStore) ListPairwiseSubjects(ctx context.Context) ([]security.PairwiseSubjectMapping, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite: list pairwise: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.PairwiseSub, &item.LocalSub); err != nil {
			return nil, fmt.Errorf("sqlite: scan pairwise: %w", err)
		}
		out = append(out, item)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("sqlite: list pairwise rows: %w", err)
	}
	return out, nil
}
...
func (s *PairwiseSubjectStore) DeletePairwiseSubject(ctx context.Context, pairwiseSub string) error {
	if _, err := s.db.ExecContext(ctx, `
        DELETE FROM pairwise_subjects WHERE pairwise_sub = ?`, pairwiseSub); err != nil {
		return fmt.Errorf("sqlite: delete pairwise: %w", err)
	}
	return nil
}
