...
func NewDeviceStore(dsn string) (*DeviceStore, error) {
	...
	if err := migrate.Run(context.Background(), db, "devices", []migrate.Migration{{Version: 1, Name: "baseline", SQL: devSchema}}); err != nil {
		return nil, err
	}
	return &DeviceStore{db: db}, nil
}
func (s *DeviceStore) Get(_ context.Context, id string) (*device.Device, error) {
	return scanDev(s.db.QueryRow(devSelect+` WHERE id=?`, id))
}
func (s *DeviceStore) GetByFingerprint(_ context.Context, u, f string) (*device.Device, error) {
	return scanDev(s.db.QueryRow(devSelect+` WHERE user_id=? AND fingerprint=?`, u, f))
}
...
func (s *DeviceStore) ListByUser(_ context.Context, uid string) ([]*device.Device, error) {
	r, e := s.db.Query(devSelect+` WHERE user_id=? ORDER BY last_seen_at DESC`, uid)
	if e != nil {
		return nil, e
	}
	defer r.Close()
...
func (s *DeviceStore) ListAll(_ context.Context) ([]*device.Device, error) {
	r, e := s.db.Query(devSelect + ` ORDER BY last_seen_at DESC`)
	if e != nil {
		return nil, e
	}
	defer r.Close()
...
func (s *DeviceStore) DeleteByUser(_ context.Context, uid string) error {
	_, e := s.db.Exec(`DELETE FROM devices WHERE user_id=?`, uid)
	return e
}

func scanDev(r interface{ Scan(...any) error }) (*device.Device, error) {
	var i, u, f, t, p, ov, bn, bv, dn, no, ua, lip, ll string
	var fsa, lsa int64
	var sp int
	var lc int
	var ts float64
...
func NewStreamStore(dsn string) (*StreamStore, error) {
	...
	if err := migrate.Run(context.Background(), db, "ssf_streams", []migrate.Migration{{Version: 1, Name: "baseline", SQL: strSchema}}); err != nil {
		return nil, err
	}
...
func (s *StreamStore) Create(ctx context.Context, st *caep.Stream) error {
	...
	if e != nil && strings.Contains(e.Error(), "UNIQUE") {
		return caep.ErrStreamExists
	}
	return e
}
func (s *StreamStore) Get(ctx context.Context, id string) (*caep.Stream, error) {
	return scanStr(s.db.QueryRowContext(ctx, strSelect+` WHERE id=?`, id))
}
func (s *StreamStore) Update(ctx context.Context, st *caep.Stream) error {
	...
	if n, _ := r.RowsAffected(); n == 0 {
		return caep.ErrStreamNotFound
	}
	return nil
}
func (s *StreamStore) Delete(ctx context.Context, id string) error {
	_, e := s.db.ExecContext(ctx, `DELETE FROM ssf_streams WHERE id=?`, id)
	return e
}
func (s *StreamStore) List(ctx context.Context, sub string) ([]*caep.Stream, error) {
	q := strSelect + ` ORDER BY created_at DESC`
	if sub != "" {
		q = strSelect + ` WHERE subject=? ORDER BY created_at DESC`
		r, e := s.db.QueryContext(ctx, q, sub)
		if e != nil {
			return nil, e
		}
		defer r.Close()
...
func strDM(s *caep.Stream) string {
	if s.Delivery != nil {
		return s.Delivery.Method
	}
	return ""
}
func strDE(s *caep.Stream) string {
	if s.Delivery != nil {
		return s.Delivery.Endpoint
	}
	return ""
}
func strDA(s *caep.Stream) string {
	if s.Delivery != nil {
		return s.Delivery.Authorization
	}
	return ""
}
func scanStr(r interface{ Scan(...any) error }) (*caep.Stream, error) {
	var i, is, su, ej, dm, de, da string
	var cat, uat int64
	if e := r.Scan(&i, &is, &su, &ej, &dm, &de, &da, &cat, &uat); e != nil {
		if e == sql.ErrNoRows {
			return nil, caep.ErrStreamNotFound
		}
		return nil, e
	}
