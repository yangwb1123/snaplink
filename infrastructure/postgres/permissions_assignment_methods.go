...
// roleStrip is a pending rewrite of one user's assignment row with a removed
// role code already filtered out.
type roleStrip struct {
	userID string
	roles  []string
}

// collectRoleStrips walks every assignment under clientID and returns the rows
// whose role list contains roleCode, with roleCode removed. Rows are read fully
// before any write so the caller can drive the rewrites on the same
// transaction without an open cursor.
func collectRoleStrips(ctx context.Context, tx *sql.Tx, clientID, roleCode string) ([]roleStrip, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: scan assignments: %w", err)
	}
...
// filterRoleCode returns roles with roleCode removed plus whether any element
// was dropped.
...
func applyRoleStrips(ctx context.Context, tx *sql.Tx, clientID string, updates []roleStrip) error {
	for _, u := range updates {
		...
		if err != nil {
			return fmt.Errorf("postgres: marshal updated assignment: %w", err)
		}
		if _, err := tx.ExecContext(ctx, `
			...
			return fmt.Errorf("postgres: strip assignment: %w", err)
		}
	}
	return nil
}
