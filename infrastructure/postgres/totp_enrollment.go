...
func (s *TOTPEnrollmentStore) ExportSeeds(ctx context.Context) ([]authenticators.SeedRecord, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: list totp seeds: %w", err)
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&userID, &secret); err != nil {
			return nil, fmt.Errorf("postgres: scan totp seed: %w", err)
		}
		out = append(out, authenticators.SeedRecord{UserID: userID, Secret: secret})
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("postgres: list totp seeds rows: %w", err)
	}
	return out, nil
}
