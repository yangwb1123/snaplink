...
| OIDC Back-Channel Logout 1.0 | `/logout`, `/end_session` | `WithBackchannelLogout(issuer, notifier)` | `backchannel_logout.go` | POSTs signed `logout_token` (typ=logout+jwt + events claim) to `Client.BackchannelLogoutURI` for the client in the bearer / id_token_hint's aud; carries `sid` claim when the inbound bearer / id_token_hint had one (sourced from the active session — see OIDC `sid` row below); fail-open (notify failure logged + audited, never blocks logout response); single-RP only today, multi-RP fan-out is future scope; `backchannel_logout_session_supported: true` advertised in discovery when `WithSessionManager` is wired (sid is sourced from the session manager) |
