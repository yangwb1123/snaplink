...
| RFC 9470 OAuth 2.0 Step-Up Authentication Challenge | resource-server helper (no AS endpoint) | always | `step_up_auth.go` | Helper `BuildStepUpChallenge` composes the `WWW-Authenticate: Bearer error="insufficient_user_authentication", acr_values="...", max_age="..."` header per §3 — resource servers in this SDK's monorepo emit it on 401 when the bearer's ACR / auth_time is too weak for the operation. SPAs catching the 401 re-initiate `/auth/login` with the requested `acr_values` + `max_age` (both already plumbed); the AS mints a stronger token. Untrusted descriptions pass through RFC 7235 quoted-string escaping so they can't break out and inject auth parameters. Empty demand (no ACR, no max_age) refused — RFC has no semantics for "demand nothing" |
