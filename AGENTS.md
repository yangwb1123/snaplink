...
| RFC 8705 §3 mTLS-bound access tokens (issuance side) | `/token` | `WithClientCertExtractor` (pluggable so reverse-proxy-terminated TLS works) | `mtls_bound.go` | When the wired extractor returns a client cert from the inbound request, the issued access token's `cnf.x5t#S256` claim carries the cert's SHA-256 thumbprint per §3.1. Resource servers MUST then verify the inbound connection's cert thumbprint matches on every protected request (the verification half is a follow-up). Token type stays `Bearer` per §3 (mTLS doesn't introduce a new type — the binding is implicit in the cnf claim). Mutually exclusive with DPoP — both populated would be a caller bug. Default extractor (`DefaultTLSPeerCertExtractor`) reads `r.TLS.PeerCertificates[0]`; envoy/nginx deployments inject a header-parsing extractor. Discovery: `tls_client_certificate_bound_access_tokens: true` when wired |
