...
| RFC 9449 DPoP (issuance side) | `/token` (header `DPoP: <jwt>`) | always accepted when header present (opt-in per request); JTI replay defense via `WithJTIReplayStore` | `dpop.go` | client supplies a fresh signed JWT (typ=`dpop+jwt`, alg=EdDSA, header `jwk` carries the public key) on every /token call it wants bound. AS validates `htm` (request method), `htu` (request URL sans query/fragment), `iat` within ±60s window, `jti` present (replay defense via store when wired). On success: RFC 7638 SHA-256 JWK thumbprint stamped into the issued access token's `cnf.jkt` claim (RFC 7800), `token_type` response flips from `Bearer` to `DPoP`. Validate() echoes the binding back as `TokenClaims.ConfirmationJKT` so resource-side verification (separate future commit) has a clean handle. Discovery advertises `dpop_signing_alg_values_supported: ["EdDSA"]`. Failures collapse to `invalid_dpop_proof` per §5.2 (oracle-resistance) |
