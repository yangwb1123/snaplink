...
| OIDC Back-Channel Logout 1.0 | `/logout` | `WithBackchannelLogout(issuer, notifier)` | `backchannel_logout.go` | POSTs signed `logout_token` (typ=logout+jwt + events claim) to `Client.BackchannelLogoutURI` for the client in the bearer's aud; fail-open (notify failure logged + audited, never blocks logout response); single-RP only today, multi-RP fan-out is future scope |
