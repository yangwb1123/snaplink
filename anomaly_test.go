...
type recordingDetector struct {
	...
	name        string
	seen        []*sso.LoginEvent
	cannedAnoms []sso.Anomaly
	cannedErr   error
}

func (r *recordingDetector) Name() string { return r.name }
func (r *recordingDetector) Inspect(_ context.Context, e *sso.LoginEvent) ([]sso.Anomaly, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (c *captureSink) Record(_ context.Context, e *sso.LoginEvent, a sso.Anomaly) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.pairs = append(c.pairs, sinkPair{Event: e, Anomaly: a})
	if c.errOnce != nil {
		err := c.errOnce
		c.errOnce = nil
		return err
...
func TestNewAsyncAnomalyRunner_NoDetectorsReturnsNil(t *testing.T) {
	...
	r := sso.NewAsyncAnomalyRunner(nil, nil)
	if r != nil {
...
func TestAsyncAnomalyRunner_DispatchesToEveryDetector(t *testing.T) {
	...
	sink := &captureSink{}
	r := sso.NewAsyncAnomalyRunner([]sso.AnomalyDetector{d1, d2}, sink)
	if r == nil {
	...
	event := &sso.LoginEvent{SubjectID: "alice", Outcome: "success"}
...
func TestAsyncAnomalyRunner_DetectorErrorsDontStopSiblings(t *testing.T) {
	...
	d2 := &recordingDetector{name: "d2", cannedAnoms: []sso.Anomaly{
		{Type: "test", Severity: sso.AnomalySeverityInfo},
	}}
	sink := &captureSink{}
	r := sso.NewAsyncAnomalyRunner([]sso.AnomalyDetector{d1, d2}, sink)
	r.Start()
	...
	r.Dispatch(context.Background(), &sso.LoginEvent{SubjectID: "alice"})
...
func TestAsyncAnomalyRunner_SinkErrorIsLoggedNotRequeued(t *testing.T) {
	...
	// + move on.
	d := &recordingDetector{name: "d", cannedAnoms: []sso.Anomaly{
		{Type: "test", Severity: sso.AnomalySeverityInfo},
	}}
	sink := &captureSink{errOnce: errors.New("audit sink down")}
	r := sso.NewAsyncAnomalyRunner([]sso.AnomalyDetector{d}, sink)
	r.Start()
	defer func() { _ = r.Close(context.Background()) }()

	r.Dispatch(context.Background(), &sso.LoginEvent{SubjectID: "alice"})
	r.Dispatch(context.Background(), &sso.LoginEvent{SubjectID: "bob"})
...
func TestAsyncAnomalyRunner_DropsNewestWhenQueueFull(t *testing.T) {
	...
	r := sso.NewAsyncAnomalyRunner([]sso.AnomalyDetector{d}, nil,
		sso.WithAnomalyQueueSize(1),
	)
...
func TestAsyncAnomalyRunner_BlockPolicyHonorsCtx(t *testing.T) {
	...
	r := sso.NewAsyncAnomalyRunner([]sso.AnomalyDetector{d}, nil,
		sso.WithAnomalyQueueSize(1),
		sso.WithAnomalyDropPolicy(sso.AnomalyDropBlock),
	)
	// Fill the queue.
	r.Dispatch(context.Background(), &sso.LoginEvent{SubjectID: "first"})

	ctx, cancel := context.WithTimeout(context.Background(), 50*time.Millisecond)
	defer cancel()
	start := time.Now()
	r.Dispatch(ctx, &sso.LoginEvent{SubjectID: "second"})
...
func TestAsyncAnomalyRunner_NilRunnerDispatchSafe(t *testing.T) {
	...
	var r *sso.AsyncAnomalyRunner
	r.Dispatch(context.Background(), &sso.LoginEvent{SubjectID: "x"})
...
func TestAsyncAnomalyRunner_DispatchAfterCloseDropsSilently(t *testing.T) {
	...
	r.Dispatch(context.Background(), &sso.LoginEvent{SubjectID: "after"})
...
func TestAsyncAnomalyRunner_MultipleAnomaliesPerEvent(t *testing.T) {
	...
	d := &recordingDetector{name: "multi", cannedAnoms: []sso.Anomaly{
		{Type: "a1", Severity: sso.AnomalySeverityWarn},
		{Type: "a2", Severity: sso.AnomalySeverityCritical},
	}}
	sink := &captureSink{}
	r := sso.NewAsyncAnomalyRunner([]sso.AnomalyDetector{d}, sink)
	r.Start()
...
func TestRecorderAnomalySink_NilRecorderReturnsNil(t *testing.T) {
	// Defensive: NewRecorderAnomalySink with nil recorder returns
	// nil so embedders can wire `NewRecorderAnomalySink(maybeNil)`
	// directly into NewAsyncAnomalyRunner.
	if got := sso.NewRecorderAnomalySink(nil); got != nil {
