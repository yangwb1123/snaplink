...
# github.com/crewjam/saml [XML/DSig] — none MUST enter the core go.mod), so CI
...
# its own `replace => ../` (or ../../).
ci-modules: ## Build + race-test the nested modules (kms/awskms, kms/gcpkms, kms/pkcs11, redis, saml).
