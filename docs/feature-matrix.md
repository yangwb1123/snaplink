...
| User-lifecycle state machine + unified end-user auth/token gate + transition revocation + optional auto-deprovision sweep | `GET`/`POST /api/v1/admin/users/:id/lifecycle`; `/auth/login`, user `/token` grants, validation and introspection | `WithUserLifecycle(store)` (+ `WithUserAutoDeprovision(cfg, activity)`); stock `user_lifecycle.enabled` uses memory, revokes sessions/refresh tokens on every non-active transition, and is rejected in multi-replica mode | `domains/userlifecycle/` + `protocols/lifecyclereactions/` + `interfaces/sso/options_admin.go` + `cmd/sso-server/build_stores.go` |
