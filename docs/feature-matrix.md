...
| Cloud workload-identity client auth (GCP; AWS/Azure follow-up) | `/token` client authentication | `WithWorkloadIdentityProviders(security.NewGCPWorkloadIdentityValidator(...))` + `Client.TokenEndpointAuthMethod=workload_identity` | `security/securityverify/workload_identity.go` + `securityverify/workload_identity_gcp.go` |
