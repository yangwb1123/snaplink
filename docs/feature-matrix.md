...
| Password history / reuse prevention | `POST /me/password`, `POST /auth/reset-password` | Explicit `WithPasswordHistoryStore` wiring (programmatic `spi.PasswordPolicyConfig.MaxHistory` controls capacity where applicable; no stock `authenticators.password.policy.max_history` YAML setting) | `interfaces/sso/server_signup.go` + `infrastructure/defaultimpl/memorystorecredential/memory_password_credentials.go` |
