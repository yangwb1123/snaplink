...
// normalized panic 500 (echo.New() attaches no Recovery), and the
// unmatched-surface byte contract.
func TestSmoke_AuthorizationCodeAndEmbedderRoute(t *testing.T) {
	...
	// Panic route: echo.New() has no framework recovery, so the SDK's
	// outermost recovery writes the normalized JSON 500.
