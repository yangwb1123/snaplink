...
| 运维场景 | 期望 | 当前缺口 |
|---|---|---|
| 某 client_id 泄密需吊销该 client 的所有 token | `DELETE /api/v1/admin/tokens?client_id=leaked-app` | ⚠️ 部分支持 |
...
| 审计要求"展示 X 时间内吊销的所有令牌及吊销原因" | `GET /api/v1/admin/tokens/revoked?since=2026-06-01` | ❌ 无吊销日志查询 API |
...
| 检查项 | 结果 |
|---|---|
| `POST /api/v1/admin/tokens/revoke` 批量吊销（按 subject/client 过滤） | ⚠️ `HandleBulkRevoke` 已存在——支持 subject/client 过滤、confirm 门禁、审计事件（`token_portfolio.go`） |
| `DELETE /api/v1/admin/sessions` 批量 session 操作 | ❌ 不存在 |
| `GET /api/v1/admin/tokens/expiring`（预过期查询） | ❌ 不存在 |
| `GET /api/v1/admin/sessions?ip=`（按 IP 查询/过滤） | ❌ SessionRecord 无 IP 字段 |
| `GET /api/v1/admin/tokens/revoked`（吊销历史查询 API） | ❌ 不存在——审计事件已记录但无可查询 API |
| `platform/slo/` 包或框架 | ❌ 目录不存在；无 SLI/SLO 聚合框架 |

### Scope

1. **SessionRecord 扩展** — `SessionRecord` 加可选字段：
...
2. **批量 session API**：
   - `DELETE /api/v1/admin/sessions` 使用 filter 参数（tenant_id, client_id, max_last_active, user_id）
   - 大结果集通过后台 Job（`platform/lifecycle/batchjob` 暂不存在——可新建）异步执行
   - 审计事件 `admin_sessions_bulk_terminated`

3. **吊销日志查询 API**：
   - `GET /api/v1/admin/tokens/revoked` — 分页 + 日期范围 + 按 admin 过滤
   - 需要新建 `RevocationLogStore` SPI 或复用已有 audit 事件存储的查询能力
...
| 工作量 | **XL** — SessionRecord 扩展（M）+ Batch Session API（M）+ 吊销日志查询（M）+ 后台 Job 框架（M）+ SLO 框架（M）|
| 独立性 | 高 — 不破坏现有 token 验证/吊销路径 |
| 建议优先级 | **P2**（高价值但工程量较大，可分批交付：先做 SessionRecord 扩展 + 吊销日志查询 → 再做批量 session API → 最后 SLO） |
...
| `infrastructure/notification/` 或 `platform/notification/` | ✅ 目录不存在 |
...
| 检查项 | 结果 |
|---|---|
| `SessionBridge` 或 `LinkedSession` 实体 ❌ 不存在 | ⚠️ `GlobalSID` + `LinkRecord` 已存在（`platform/lifecycle/sessionhub/`） |
| 跨协议 Session 关联机制（`session_bridge_id` / `linked_session_id`） | ⚠️ `GlobalSID` + `Coordinator.Link()` 已实现（`sessionhub/coordinator.go`） |
| SAML SLO 触发 OIDC `/end_session` | ⚠️ `Coordinator.Logout()` 在销毁 core session 后调用 `OIDCLogoutTrigger.TriggerBackchannelLogout()` |
| OIDC BCL 触发 SAML Session 终止 | ⚠️ `Coordinator.Logout()` 调用 `SAMLLogoutTrigger.Fanout()` |
| 跨协议 Session 统一查询 API | ✅ 不存在——LinkStore 无管理面查询接口 |
| 仓库中 `cross.*protocol.*session|session.*bridge|protocol.*bridge|SAML.*OIDC.*session|oidc.*saml.*session|bridge.*session|session.*link.*protocol|LinkSession|LinkedSession` 匹配 | ⚠️ `sessionhub` 包存在——`GlobalSID`, `LinkRecord`, `Coordinator` 均已实现 |

### Scope

已存在的基础（复用 `platform/lifecycle/sessionhub/`）：

- `GlobalSID`（UUID v7 风格，实际上使用 crypto/rand 32 字节）
- `LinkRecord`（ProtocolCore + ProtocolSAML 目前）
- `Coordinator.Link()` / `Coordinator.Logout()`（跨协议登出传播）
- `OIDCLogoutTrigger`（Back-Channel Logout fan-out）
- `SAMLLogoutTrigger`（SAML SLO fan-out）
- `MemoryLinkStore`（内存链路存储）

剩余缺口：

1. **管理面统一 Session 查询 API**：
   - `GET /api/v1/admin/sessions/unified?user_id=x` — 返回该用户在 OIDC + SAML + CIBA 下的所有 session，按 `GlobalSID` 分组
   - 需要 `LinkStore.ListBySubject(ctx, subject)` 方法（当前仅有 `List(ctx, gsid)`）

2. **用户面跨协议 Session 视图扩展**：
   - `GET /me/sessions` 可选参数 `include_linked=true` 显示关联的 SAML session
   - User Portal 在 session 列表中标识 SAML 关联 session

3. **SAML SLO handler 集成验证**：
   - `infrastructure/saml/saml.go` 中的 `linkGlobalSession` 调用了 `Coordinator.Link()`—验证是否正确集成
   - 确保 SAML IdP-initiated SLO 路径调用 `Coordinator.Logout()`

4. **OIDC → SAML 注销传播循环防护**：
   - 现有 `Coordinator` 中 OIDC 注销触发 SAML fan-out 后再触发 OIDC 注销会形成循环
   - 需要协议标记来防止反向传播（`OIDC → SAML` 不复发回 OIDC）
...
| 通过非桥接路径建立的 session（如直接从 OIDC 登录的用户没有 SAML 侧） | 无 `GlobalSID` 时的行为字节等同于当下 |
| 大规模租户的跨协议关联开销 | `GlobalSID` 写入时间在 10ms 内；查询通过 `gsid` 主键或 `subject` 索引 |
...
| 技术复用 | **高** — `sessionhub` 已提供核心引擎，仅需补齐管理面 API 和 UI |
| 工作量 | **S-M** — 管理面查询 API（S）+ 用户面扩展（S）+ 集成验证（M）|
| 独立性 | 中 — 依赖于已有的 sessionhub 包正确 wiring |
| 建议优先级 | **P2**（核心引擎已完成，剩余为管理面补齐，适合作为短 sprint 交付） |
...
|---|---|---|
| 密码过期轮换 | ⚠️ `MaxAgeDays` 已定义但未在登录时执行 | 许多框架要求 90/180 天强制轮换 |
| 密码历史（避免重复使用） | ✅ `PasswordHistoryStore` 已实现（`stored_password.go`） | 不得重复使用最近 N 个密码 |
| 密码复杂度策略 | ✅ `PasswordPolicyValidator` + `NewPasswordPolicyValidator` 已实现（`reg_gate.go`） | 已满足 |
| 登录时强制改密（首次/重置后） | ❌ `ForcePasswordChange` 标记不存在 | 首次登录 / 管理员重置密码后须强制用户改密 |
| 密码过期前通知 | ❌ 无 `PasswordExpiresAt` 检查 | 过期前 7/14 天提醒用户 |
| 密码强度评估面板 | ⚠️ HIBP + 弱口令字典存在但无用户面展示 | 用户应能看到密码强度评分 |
| 依角色/租户不同密码策略 | ⚠️ `PasswordPolicyConfig` 在代码级支持但无租户级覆盖 | 不同安全等级的租户适用不同策略 |
| 密码年龄作为风险信号 | ❌ 密码年龄不接入风险评估 | 老密码 = 高风险，影响信任评估 |
...
| `PasswordHistoryStore` SPI | ⚠️ `PasswordHistoryStore` 已存在（`domains/authenticators/stored_password.go`）含 `Record()` + `CheckHistory()`，`MemoryPasswordHistoryStore` 已实现 |
| `PasswordPolicyValidator` 引用实现 | ⚠️ `NewPasswordPolicyValidator(cfg PasswordPolicyConfig)` 已实现（`shared/spi/reg_gate.go`）含 minLen/upper/lower/digit/special |
| `ForcePasswordChange` 标记 | ✅ User 实体无此标记 |
| 登录时检查 `password_expired` | ✅ `handleLogin` 从不检查密码年龄 |
| 仓库中 `password.*expir|password.*history|password.*reuse|ForcePasswordChange|PasswordExpiresAt|password.*age|password.*rotate` | ⚠️ `PasswordHistoryStore` 存在；`MaxAgeDays` 在 config 中定义但未在登录路径执行 |
...
2. **PasswordHistoryStore 集成**（已有 `MemoryPasswordHistoryStore`，需确保引用集成在密码修改路径）：
   - 改密时调用 `PasswordHistoryStore.Record()`（需确认现有路径是否已集成）
   - 改密时调用 `PasswordHistoryStore.CheckHistory()`（需确认现有路径是否已集成）
   - SQLite 持久化实现（当前仅有 Memory 实现）
...
6. **用户面改密流程增强**：
   ...
   - 改密时检查 `PasswordHistoryStore.CheckHistory()` → 如返回 true 则拒绝（"你不能重复使用最近 5 次用过的密码"）
...
| 技术复用 | **高** — 复用现有 `PasswordPolicyValidator`（已有引用实现）、`PasswordHistoryStore`（已有 interface + 内存实现）、`stored_password.go`（认证）、`emailsmtp`（通知）、`UserProvider` |
| 工作量 | **M** — User 扩展 + SPI（M）+ PasswordHistoryStore SQLite 实现（S）+ 登录检查（M）+ Admin API（S）|
| 独立性 | 高 — 不改变现有密码认证核心路径 |
...
| ④ 跨协议会话桥接 | 中-高 | 高 | S-M | **P2** | 第 3-4 月 | 核心引擎 sessionhub 已完成，需补齐管理面 |
| ⑤ 密码生命周期策略管理 | 高 | 高 | M | **P1** | 第 2-3 月 | 已有 PasswordHistoryStore + PolicyValidator 需集成 |

### 一句话推荐

**先并行方向①（认证流水线——差异化能力）+ 方向③（通知基建——用户可见性，3-4 周）→
方向⑤（密码策略——合规刚需，4-6 周）→ 方向④（会话桥接，2-3 周）→ 方向②（批量治理，
分批交付，5-8 周）。**
...
- **无 `RevocationLogStore` 查询 API** — 吊销操作无法追溯"谁在什么时候因为什么吊销了什么"（审计事件已记录但无可查询接口）
- **Token 签发无 P99 延迟 SLO** — Prometheus 指标已有但未聚合为可告警的 SLO

### 配置 / 部署层面
- **`MaxAgeDays` 在 `PasswordPolicyConfig` 中已定义但未在登录路径使用** — 需要将配置值接入 `UserPasswordMeta` 判断
- **`WithPasswordPolicyValidator` 已支持** — SDK 用户可调用 `WithPasswordPolicy(NewPasswordPolicyValidator(cfg))`
