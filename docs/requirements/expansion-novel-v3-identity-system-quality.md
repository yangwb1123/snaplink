...
| 出站调用 | 位置 | 现有 SSRF 防护 |
|---|---|---|
| Federation 信任链获取 | `domains/federation/fetch.go` | 无（URL 来自上游声明） |
| SAML 元数据刷新 | `infrastructure/saml/idp/metadata.go` | 仅 HTTPS 检查 |
...
| 防护机制 | 代码命中 | SSRF 覆盖面 |
|---|---|---|
| `isHTTPSURL\|scheme == "https"\|MustHTTPS` | 仅在 `saml/idp/` 存在 | 仅覆盖 SAML |
| `AllowedRequestURIs\|allowedRequestURIs` | `server_login_gates.go` + JAR | 仅覆盖 JAR |
| `DialContext\|DialTLS\|net.Dialer\|net.Conn` 自定义 Dialer | **0**（所有出站调用使用默认 `http.Client`） | 无法阻止 TCP-level SSRF |
