...
| `domains/federation` | 23 | monolithic-shared-state | 11 of 24 files carry methods on `TrustChainResolver`/`RegistrationClientStore`; `trust_marks`/`registration` call back into the resolver (cycle). The metadata-policy combine/enforce engine was extracted to a metadatapolicy sub-package; the remaining method-bearing core stays. |
