...
snaplink ships as an embeddable SDK with **no required external SaaS
dependency**.
...
1. **Every concern is an interface (an SPI) plus at least one real
   implementation.** Interfaces and shared types live in `shared/core` /
   `shared/spi` (or the owning lower package); implementations live in
   `infrastructure/defaultimpl` and per-backend packages.
...
4. **No fan-in cap.** A shared SPI/types kernel is supposed to be widely
   imported. God-object risk is policed by fan-OUT discipline (the root cannot
   hold business logic, ADR-0001) and by the size/complexity budgets (ADR-0005),
   not by penalizing reuse.
5. **Nested modules**
   (`infrastructure/{saml,ldap,kerberos,radius,extauthz,kafka,mqtt,kms/*}` and
   `cmd/{sso-mcp,sso-operator}`) keep their own `go.mod` — that boundary is the
   isolation seam for optional third-party dependencies. Redis and Postgres are
   root-module packages. Nested modules are built/tested by `make ci-modules`.
...
`ci-modules`); contributors must place new shared types in `shared/core` or
`shared/spi`, not root.

**Risks** — a new concern implemented only as a mock, or with infra types in
the domain package. Mitigated by the convention plus
`python cli.py check-invariants` and code review
(`../review-checklist.md`).

## Enforcement

Convention + `python cli.py check-invariants` (`checks/invariants.py`) +
ADR-0002 boundary gate. Module map of record:
[AGENTS.md §2](../../AGENTS.md#2-module-map). Manifest:
