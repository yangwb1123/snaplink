...
| `password_policy_violation` | 400 | `POST /auth/register`, `POST /me/password`, `POST /auth/reset-password`, or `POST /api/v1/admin/users/:id/password` supplied a password rejected by the wired `PasswordPolicyValidator` | Choose a password meeting the operator's policy |
