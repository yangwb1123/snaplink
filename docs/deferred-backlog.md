...
| TypeScript/Python/Rust/PHP SDKs | **Implemented** | TypeScript/Python clients are generated from `docs/openapi.yaml` via the `ops/build/sdk-surface.json` registry (full documented operation set: admin, SCIM, SSF, Federation included); Rust/PHP remain independent SDK packages with their own release boundaries. `python cli.py sdk-surface check` validates the current contract and the four fixed SDK manifest versions, including the TypeScript `package-lock.json` root, while `python cli.py sdk-surface versions` exposes the same read-only gate. `python cli.py sdk-surface diff --baseline-ref <ref>` (or `--baseline-file <path>`) enforces operation and bounded `components.schemas` compatibility against an explicit local baseline. A registry file is schema-unavailable unless explicitly paired with `--baseline-openapi-file`; ref baselines read both files from the same local commit. Versioned package publication remains an external boundary handled by registry-specific workflows; the diff is not a complete vendor-level OpenAPI comparison. |
...
both. The explicit-baseline `python cli.py sdk-surface diff` gate also reports
operation removals/relocations and bounded schema/type changes; it is not a
complete vendor-level OpenAPI diff. The configured replica's admin endpoint inventory
