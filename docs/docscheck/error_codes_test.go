...
// sentinelTableDirs are the Go packages docs/error-codes.md's two
// "Sentinel"-headed tables document (both self-describe as "SDK Go errors,
// not HTTP wire codes" — a caller branches on them with errors.Is, so the doc
// lists the Go IDENTIFIER itself, not a derived wire string):
// interfaces/ssoclient/rs (resource-server token validation) and
// shared/security/securityverify (outbound webhook signature verification).
// Identifiers are matched against the UNION of both dirs rather than
// per-table (the two tables don't share any identifier names in practice, so
// this is simpler than threading a table->dir association through
// scanDocTableByHeader for no extra safety).
var sentinelTableDirs = []string{"interfaces/ssoclient/rs", "shared/security/securityverify"}
