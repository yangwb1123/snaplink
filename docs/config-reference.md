...
| `activation.{backend,ticket_ttl,codes}` | Optional product activation for the hosted-login SDK. `backend: memory` seeds one-time license/invitation credentials from `codes[]` and mounts `/api/v1/activation/prepare`, `/api/v1/me/activation/claim`, and `/api/v1/me/account-context`; empty backend leaves them absent. `codes[].license_key` and `codes[].invitation_code` are mutually exclusive credential fields; use `secret://` references or an equivalent secret source. The memory backend is single-replica only, retains credential digests rather than plaintext, and returns authoritative entitlement context after the authenticated claim. |
...
| Product activation codes | `activation.backend` | off (`""`) · `memory` (single replica) |
