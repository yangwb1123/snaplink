...
| `feature_gates.admin_api` / `feature_gates.web_spa` | `interfaces/sso` mounts BOTH surfaces unconditionally at `Mount()` time now and gates reachability with a request-time check (`shared/core.GatedRouter` for `/api/v1/admin/*`, `shared/core.GateHTTPHandler` for the SPA static-asset mounts) instead of deciding "mount or don't" once, at boot. `SetAdminAPIGateHook` / `SetWebSPAGateHook` (wired to `Server.SetAdminAPIGateEnabled` / `SetWebSPAGateEnabled`) flip that check live, in BOTH directions, with no restart — gate-off answers `http.NotFound` byte-identically to a path that was never registered. One asymmetry: `SetWebSPAGateEnabled` reports no effect (`ignored_requires_restart`) when NO SPA filesystem (admin console / hosted login / portal / developer portal) was ever wired via a `With*FS` option at boot — a live gate can only suppress/reveal an already-mounted route, never conjure a filesystem that doesn't exist. `admin_api` has no such gap (its group always has at least the client lookup + endpoint inventory to gate) |
| Every OTHER `feature_gates.*` field (`oidc`/`ciba`/`caep`/`federation`/`self_service`), storage backends, `server.listen`, TLS material, cluster/etcd endpoints, … | Detected if changed, reported under `ignored_requires_restart`, and left COMPLETELY untouched — never silently misapplied. These route groups are still decided once at `Mount()` with no live re-check, so toggling one after boot cannot add/remove already-registered mux routes without a full re-Mount, which this SDK does not support at runtime. See `config/reload`'s package doc for the full rationale |
