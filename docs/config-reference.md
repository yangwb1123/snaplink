...
| Everything else (`feature_gates.*`, storage backends, `server.listen`, TLS material, cluster/etcd endpoints, …) | Detected if changed, reported under `ignored_requires_restart`, and left COMPLETELY untouched — never silently misapplied. `feature_gates.*` looks "safe" (a toggle, no store/connection to reprovision) but isn't wired: feature-gated route groups are decided once at `Mount()`, and toggling a gate after boot cannot add/remove already-registered mux routes without a full re-Mount, which this SDK does not support at runtime. See `config/reload`'s package doc for the full rationale |
