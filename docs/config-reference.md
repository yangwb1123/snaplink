...
| `clients[].token_endpoint_auth_method` | OAuth client authentication method: `client_secret_basic` (default when omitted), `client_secret_post`, `private_key_jwt`, `tls_client_auth`, `self_signed_tls`, or `none`. `none` is the public-client contract: `clients[].secret` MUST be empty and `clients[].require_pkce` MUST be `true`; unsupported values and violations fail config validation. When a configured public client already exists in the durable store, boot reconciliation upgrades only a legacy empty method, preserves existing client policy/keys, and refuses to convert a secret-bearing client. |
...
| `server.topology.{mode,allow_per_pod_state}` | `mode: single` declares one process; `mode: multi` makes per-process OAuth, session, replay, identity, CIBA, MFA challenge, pairwise-subject, identity-link, ReBAC, and enabled user-lifecycle stores a boot error. It also rejects `client_secret_rotation.enabled: true` when `allow_per_pod_state: false` because a scheduler owner/coordination mechanism is required (the stock server has neither). `allow_per_pod_state: true` is an explicit development-only escape hatch and is rejected unless mode is `multi`. |
...
| `activation.{backend,ticket_ttl,codes}` | Optional product activation for the hosted-login SDK. `backend: memory` or `postgres` seeds one-time license/invitation credentials from `codes[]` and mounts `/api/v1/activation/prepare`, `/api/v1/me/activation/claim`, and `/api/v1/me/account-context`; empty backend leaves them absent. `codes[].license_key` and `codes[].invitation_code` are mutually exclusive credential fields; use `secret://` references or an equivalent secret source. Both backends retain credential digests rather than plaintext and return authoritative entitlement context after the authenticated claim; memory is single-replica only, while postgres uses the shared Postgres/CockroachDB pool. |
...
| `security.trusted_proxies.{cidrs,hops}` | CIDR allowlist compiled once (`peertrust.Checker`) gating proxy-supplied input on the direct peer (`RemoteAddr`). `TrustedProxies` stamps one canonical client IP for rate limiting, the Geo default/risk path, audit, and push callbacks; it also gates `X-Forwarded-Host/Proto` (issuer/discovery/registration/DPoP `htu`, tenant domain routing), mesh `X-Auth-*`, serving-region headers, and header-backed mTLS certificates. Untrusted peer ⇒ direct IP/Host/TLS or the consumer's normal unauthenticated fallback. Without the canonical context, Geo's default and rate limiting use `RemoteAddr`; audit, push, and other legacy forwarded-header consumers retain their documented first-hop behavior. The resource-server SDK accepts the same middleware through `rs.Config.TrustedProxies`. Unset trusted proxies is unsafe at an untrusted edge |
...
| `security.cors.{enabled,allowed_origins,allowed_methods,allowed_headers,exposed_headers,allow_credentials,max_age,path_overrides}` | CORS is installed only when `enabled: true` and either the default `allowed_origins` list or `path_overrides` is non-empty; an empty default list with no overrides is identity/no CORS. Origins are exact matches unless the list contains `*`; with `allow_credentials: true`, `*` echoes the request origin as required by the CORS specification. `path_overrides` maps URL path prefixes to one-level policies; the longest matching prefix wins, each key must start with `/`, and an override's `enabled` field is ignored (an empty `allowed_origins` entry emits no CORS headers for that prefix). Changes require a restart; this block is not in the SIGHUP hot-reload set |
...
| `geo.backend` | IP → geo enrichment middleware's `geo.Provider`: `static` (default; in-process operator-curated CIDR table) or `maxmind` (loads the fixed local `.mmdb` named by `geo.maxmind.mmdb_path` at startup). `geo.static.entries` is always the longest-prefix static overlay, so it wins over MaxMind for matching addresses; other addresses fall through to MaxMind and misses return `geo.ErrNotFound`. Unknown backends and MaxMind read/empty/parse failures fail startup. The stock server does not download, checksum, or refresh GeoIP data automatically |
| `geo.maxmind.mmdb_path` | Required when `geo.backend: maxmind`; fixed local filesystem path read only during startup. The operator supplies a valid `.mmdb`; request input, URL fetching, checksum verification, and background refresh are not part of this setting |
...
| `audit.notary.key_file` | Required non-empty absolute path when enabled. The operator must provision a PKCS#8 PEM `PRIVATE KEY` containing an Ed25519 key; the key is independent of `keys.signing`, is never generated or stored in YAML, and private-key material is never logged |
...
| Key | Effect |
|---|---|
| `client_secret_rotation.enabled` | Enables scheduled rotation for active confidential clients. The configured client store must implement due-listing and overlap rotation; memory, SQLite, Postgres, Redis, and the built-in cache/federation decorators do. In declared `server.topology.mode: multi` with `allow_per_pod_state: false`, startup fails closed before scheduler/domain wiring because the stock server has no single scheduler owner or coordination mechanism. |
