...
| G5b（B4-4 SDK 合并序） | B4-4 strict binder（cmd-sso-ctl-entitiescmd 方向）→ B4-4 SDK form-emission（cmd/gensdk 方向） | ① sibling 方向 design gate PASS（run c399c070 重跑，当前 FAIL）为 gensdk 方向实现/合并的前置；② binder 实现先合并，gensdk 实现后合并（同批，不 release 中间态）；③ SDK 工件（`cmd/gensdk/*`、`docs/sdks/*`）由 gensdk 工作流独占，binder 变更不得触碰（reconciliation D1）；④ 机械回退：gensdk 变更携带 `TestSdkForm_PARClaimsThreaded`（claims-positive PAR E2E，**不 skip**，F1 decoder 落地前必红）→ 先于 binder 合并则 CI 无法绿（D6） |
...
**要点**：① 批次间严格按 v2 rollout 顺序（sink → aero-id → aero-vault → IdP → aero-im → console），仅 B4-3/B4-4 与 B2/B3 可并行；B4-4 内部严格 binder 先于 SDK form-emission 合并（G5b）；② 所有验收断言均映射审查测试编号（T-1.1 至 T-12）与 drill（D1），可逐条回归；③ aero-vault B3-5 与 aero-im B5-2 的"scope 对齐/配给"依赖 B4-2（IdP scope registry），这是 F2 契约修订后的唯一剩余依赖链。
