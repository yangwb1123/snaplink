...
func TestJWTIssuer_RoundTrip(t *testing.T) {
	...
	tok, err := j.Issue(context.Background(),
		&sso.Subject{ID: "u-alice", Claims: map[string]string{"team": "eng"}},
		[]string{"read"},
	)
	if err != nil {
	...
	if tok.TokenType != sso.TokenTypeBearer {
...
func TestJWTIssuer_DefaultsApplied(t *testing.T) {
	j := NewJWTIssuer()
	if j.issuer != sso.DefaultIssuer {
		t.Errorf("issuer = %q, want %q", j.issuer, sso.DefaultIssuer)
	}
...
func TestJWTIssuer_GeneratesUniqueTokens(t *testing.T) {
	...
	for i := range 20 {
		...
		tok, _ := j.Issue(context.Background(), &sso.Subject{ID: subjectN(i)}, nil)
...
func TestJWTIssuer_ValidateExpired(t *testing.T) {
	...
	tok, _ := j.Issue(context.Background(), &sso.Subject{ID: "u"}, nil)
