...
// bindOAuthParams reads OAuth request parameters from either a JSON
...
func bindOAuthParams(ctx HandlerContext, v any) error {
	r := ctx.Request()
...
func formIntoStruct(form url.Values, v any) error {
	...
	if rv.Kind() != reflect.Ptr || rv.IsNil() || rv.Elem().Kind() != reflect.Struct {
		return errors.New("sso: bindOAuthParams target must be *struct")
	}
	rv = rv.Elem()
	rt := rv.Type()
